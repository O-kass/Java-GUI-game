...
public class Game 
{
    ...
    private void createRooms()
    {
        ...
        // create the rooms
        entrance = new Room("are at the bottom of a dark pit...", items, characters, background = image.getImage("Beginning")); 
        rooms.add(entrance);

        tunnel = new Room("are crawling a slimy tunnel with a metal door at the end", items, characters,background = image.getImage("Tunnel"));
        rooms.add(tunnel);

        armory = new Room("have entered room filled with weapons, one cuboard and a chest you can equip a Green sword", items, characters,background = image.getImage("Armoury"));
        rooms.add(armory);

        chest = new Room(" have opened a chest with a golden bow and silver arrows you can only carry one weapon at a time...", items, characters,background = image.getImage("Chest"));
        rooms.add(chest);

        cupboard = new Room(" are opening a cuboard with an ancient book you can also equip this.", items, characters,background = image.getImage("Cuboard"));
        rooms.add(cupboard);

        ladder = new Room("have begun to climb a ladder with a Green glowing object at the top", items, characters,background = image.getImage("Ladder"));
        rooms.add(ladder);

        glow = new Room("have chosen to reach out only to feel a sharp jolt of pain causing you to become injured defeat this to escape", items, characters,background = image.getImage("Glow")); //not added to rooms DWARF could be in as this is the win condition

        door = new Room("pass through a large door with a fountain inside it", items, characters,background = image.getImage("Door"));
        rooms.add(door);

        fountain = new Room("are in the fountain of good health drink up to ease your pain", items, characters,background = image.getImage("Fountain"));
        rooms.add(fountain);

        book = new Room("are reading the ancient book of the underworld, BE WARY TRAVELLER of the orb of chaos " + 
            "many succumb to it's might, to overpower it you must use it's own energy but be aware of it's minions", items, characters,background = image.getImage("Book"));
        rooms.add(book);

        store = new Room("are speaking to Ernie:  Welcome to underworld, you are in trouble friend but fear not, I'm here to help equip this free potion on me...(type equip friend)", items, characters, image.getImage("Store"));
        rooms.add(store);
        //no need to add this to rooms as having both characters(Ernie and DWARF) in the terminal is too much input for user

        TELEPORT = new Room("are in THE TELEPORT ROOM", items, characters, image.getImage("Teleport"));
        ...
        MagicTeleport();
        TELEPORT.setExit("?", getRandomRoom());
        
        currentRoom = entrance;  // repeated as magic teleport overwrites it before
    }
    ...
    public void healthChecker()
    {
        ...
        else if (checker.contains("drink")){
            health = 100;
        }
    }
    ...
    private void goRoom(Command command) 
    {
        ...
        if (nextRoom == null ) {
            ...
            healthChecker(); //calls method to check if the room takes away health or not
